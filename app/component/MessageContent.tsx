...
const parseMessageContent = (content: string): ParsedContent => {
  ...
  if (content.includes("<Thinking>\n</Thinking>")) {
    const [thinking, ...rest] = content.split("<Thinking>\n</Thinking>")
    return {
      thinking: thinking.replace("<Thinking>", "").trim(),
      visible: rest.join("</Thinking>").trim(),
    }
  }
  ...
  if (content.includes("<Thinking>")) {
    return {
      thinking: content.replace("<Thinking>", "").trim(),
      visible: "",
    }
  }
  ...
  return (
    <div className="mt-4 space-y-6">
      {images.map((image, index) => {
        ...
        const imageSource =
          image.url ||
          (image.data.startsWith("http") ? image.data : `data:${image.mimeType || "image/png"};base64,${image.data}`)

        // Log the image source being used
        console.log(
          `🔗 Image ${index} source type: ${
            image.url ? "URL from url field" : image.data.startsWith("http") ? "URL from data field" : "base64 data"
          }`,
        )
            ...
            <div className="flex justify-center items-center pt-6 pb-8 px-4">
              ...
              <div
                className={`max-w-full relative ${
                  isDirectBase64 && (!imagesLoaded[index] || errorStates[index])
                    ? "hidden"
                    : (!imagesLoaded[index])
                      ? "invisible"
                      : ""
...
const InlineCode = ({ children }: { children: React.ReactNode }) => {
  return (
    <code className="px-1.5 py-1 break-words">
      {children}
    </code>
  )
}
      ...
      <div className={`overflow-auto p-1 text-sm code-block`}>
        {highlighted ? (
          <pre className="m-0 p-0">
            <code dangerouslySetInnerHTML={{ __html: highlighted }} className={`hljs ${isDark ? "dark" : ""}`} />
          </pre>
        ) : (
          <pre className="m-0 p-0">
...
const PreBlock = ({ children, ...props }: { children: React.ReactNode; [key: string]: any }) => {
  if (React.isValidElement(children) && typeof children.type === "string" && children.type === "code") {
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  {attachment.size && attachment.type && !attachment.name.endsWith(attachment.type.split("/")[1])
...
const MessageContent: React.FC<MessageContentProps> = memo(({ content, role, images, attachments, provider, onQuote, completed }) => {
  ...
  React.useEffect(() => {
    const hide = (e: MouseEvent) => {
      if (quoteMenuRef.current && e.target instanceof Node && quoteMenuRef.current.contains(e.target)) {
  ...
  return (
    <div className="relative" ref={contentRef} onMouseUp={handleMouseUp}>
      ...
      {role === "user" && attachments && attachments.length > 0 && <FileAttachmentView attachments={attachments} />}
      ...
      {images && <GeneratedImageView images={images} provider={provider} />}
