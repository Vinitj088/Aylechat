...
const prefetchAPI = async (modelId: string) => {
  try {
    ...
    fetch("/api/ai", {
      ...
      body: JSON.stringify({ warmup: true, selectedModel: modelId }),
      cache: "no-store",
    }).catch(() => {})
  } catch (e) {}
}
...
const ChatInput = forwardRef<ChatInputHandle, ChatInputProps>(
  (
    ...
    const handleModelChangeWithPrefetch = useCallback(
      (modelId: string) => {
    ...
    const handleInputChangeWithCommandDetection = useCallback(
    ...
    // FIXED: Modified submit handler to use the parent's handleSubmit directly
    const handleSubmitWithAttachments = (e: React.FormEvent) => {
      e.preventDefault()

      console.log("ChatInput: Submitting with input:", input)
      console.log("ChatInput: Attachments:", attachments.length)
    ...
    const getPlaceholder = () => {
      ...
      switch (commandMode) {
        ...
        default:
          return "Press / to ask a question..."
      }
    }
    // --- End Determine ---
        ...
        <div className="w-full bg-[var(--secondary-faint)] border border-[var(--secondary-darkest)] rounded-lg shadow-lg p-3 relative">
          <form onSubmit={handleSubmitWithAttachments} className="relative flex flex-col w-full">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center flex-shrink overflow-hidden max-w-[65%] sm:max-w-none">
                <label
                  ...
                  className="text-sm text-[var(--text-light-muted)] mr-2 hidden sm:inline font-medium"
                >
                  Model:
                </label>
            ...
            <div className="mt-1 text-[10px] text-[var(--text-light-muted)] text-center">
              Press{" "}
              <kbd className="px-1 py-0.5 bg-[var(--secondary-darker)] rounded text-[var(--text-light-default)] font-mono">
                Shift
              </kbd>{" "}
              +{" "}
              <kbd className="px-1 py-0.5 bg-[var(--secondary-darker)] rounded text-[var(--text-light-default)] font-mono">
